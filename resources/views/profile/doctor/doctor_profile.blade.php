        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="bg-gray-100 p-4 rounded-lg">
                    <div class="text-center mb-4">
                        @if($professional->doctorInfo && $professional->doctorInfo->profile_photo)
                            <img src="{{ Storage::url($professional->doctorInfo->profile_photo) }}" alt="Profile Photo" class="w-32 h-32 rounded-full mx-auto mb-4 object-cover">
                        @else
                            <div class="w-32 h-32 rounded-full bg-gray-300 flex items-center justify-center mx-auto mb-4">
                                <span class="text-3xl text-gray-600">{{ substr($professional->doctorInfo->full_name ?? $professional->user->name, 0, 1) }}</span>
                            </div>
                        @endif
                        <h2 class="text-xl font-semibold">{{ $professional->doctorInfo->full_name ?? $professional->user->name }}</h2>
            ...
            <div class="md:col-span-2">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    <div>
                        <h3 class="text-lg font-semibold mb-2 text-gray-700">Contact Information</h3>
                        <div class="mb-2">
                            <span class="font-medium">Phone:</span>
                            <span class="text-gray-600">{{ $professional->doctorInfo->phone_number ?? 'Not provided' }}</span>
                        </div>
                        <div class="mb-2">
                            <span class="font-medium">Workplace:</span>
                            <span class="text-gray-600">{{ $professional->doctorInfo->workplace ?? 'Not provided' }}</span>
                        </div>
                        <div>
                            <span class="font-medium">Address:</span>
                            <span class="text-gray-600">{{ $professional->doctorInfo->address ?? 'Not provided' }}</span>
                        </div>
                    </div>

                    <div>
                        <h3 class="text-lg font-semibold mb-2 text-gray-700">Professional Details</h3>
                        <div class="mb-2">
                            <span class="font-medium">Education:</span>
                            <span class="text-gray-600">{{ $professional->doctorInfo->education ?? 'Not provided' }}</span>
                        </div>
                        <div class="mb-2">
                            <span class="font-medium">Experience:</span>
                            <span class="text-gray-600">{{ $professional->doctorInfo->years_experience ? $professional->doctorInfo->years_experience . ' years' : 'Not provided' }}</span>
                        </div>
                        <div>
                            <span class="font-medium">Languages:</span>
                            <span class="text-gray-600">{{ $professional->doctorInfo->languages ?? 'Not provided' }}</span>
                        </div>
                    </div>
                </div>
